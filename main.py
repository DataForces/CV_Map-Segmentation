...
import cv2, glob, random, os, datetime, time
...
def stick_all_train(raster_images, vector_images):
    ...
    if rows < cols:
        ...
        row_num = num
        col_num = num + 1
    if row_num * col_num > len(raster_images):
        row_num = num
        col_num = num 
...
def result_plot_and_save(ori_image, ori_mask, ori_image_name, model_name, result, result_path, inter_over_unions, accuracy): 
    """
    ...
    fig = plt.figure()
    plt.suptitle('Image ' + ori_image_name + ' segmentation result' + '\n' + 'Inter_over_Union: '+'%.2f'%(inter_over_unions*100)+'%' \
                 + '; Over_Acc: '+'%.2f'%(accuracy*100)+'%', y = 0.8, fontsize=11)
    plt.subplot(1,3,1)
    plt.xlabel('ori_image')
    plt.imshow(ori_image[:,:,:3])
    ...
    fig.savefig(os.path.join(separate_result_file, ori_image_name + '_'+ model_name +'_compared_result.png'))
    plt.imsave(os.path.join(separate_result_file, ori_image_name + '_' + model_name +'_result.png'), result, vmin=0, vmax=11)
    plt.show()
...
if __name__ == '__main__':
    ...
    model_path = '../Model/'
    result_path = '../Result/'
#    model_name = '2017-05-20-19-17'
    model_name = '2017-09-17-17-23'
    ...
    if model_train == True:
        ...
        stacked_raster, stacked_vector = stick_all_train(raster_images[:40], vector_images[:40])
        ...
        History = model.fit(x = train_X, y = train_y, batch_size=BATCH_SIZE, nb_epoch = EPOCH, verbose=1, validation_split=CV_RATIO)
        time_end_train = time.time()
        save_model(model, model_path, model_name)
        acc_loss_visual(History.history, result_path, script_name, model_name)
    else:
        model = read_model(model_path, model_name)
        
    ############################## image testing ###########################
    test_images = raster_images
    test_images_name = raster_images_name
    time_beg_test = time.time()
    
    inter_over_unions_list = []
    accuracy_list = []
    ...
    ############################## result ##################################
        ...
        inter_over_union = intersection_over_union(vector_images[i], seg_result, smooth)
        inter_over_unions_list.append(inter_over_union)
        
        accuracy = overall_accuracy(vector_images[i], seg_result)
        accuracy_list.append(accuracy)
        
        result_plot_and_save(test_image, vector_images[i], raster_images_name[i], model_name, seg_result, result_path, inter_over_unions_list[i], accuracy_list[i])
    time_end_test = time.time()
    
    ################################ log #####################################
    if model_train == True:
        log_write(result_path, time_global, script_name, vector_images, PATCH_SIZE, N_ClASS, TRINING_SAMPLES, BATCH_SIZE,\
